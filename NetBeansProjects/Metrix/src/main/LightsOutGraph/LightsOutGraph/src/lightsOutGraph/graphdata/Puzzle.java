...
// initialise just before solving / playing
	...
	public void optimise( int[] solution ){
		...
		for( int pat=0; pat<numpat; pat++ ){
			...
			for( int i=0; i<n; i++ ) current[i] = solution[i];	// original solution
			...
			if( bestCount < 0 || bestCount > count ){
				bestCount = count;
				for( int i=0; i<n; i++ ) best[i] = current[i];   // update best
			}
		}
	...
	public int getNumQuiet(){
		if( matrix.getQuiet()==null ) return 0;
		int ns = graph.getNumStates();
      int nlty = getNullity();
...
//	all text io stuff
   ...
   public boolean hasUnsavedChanges(){ return !saved; }
